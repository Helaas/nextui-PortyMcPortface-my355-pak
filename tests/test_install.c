...
int main(void) {
    ...
    assert(strstr(file_content, "export controlfolder=\"$XDG_DATA_HOME/PortMaster\"") != NULL);
    assert(strstr(file_content, "runtime_root=\"$XDG_DATA_HOME\"") != NULL);
    ...
    assert(strstr(file_content, "export PATH=\"$runtime_root/bin:$PATH\"") != NULL);
    assert(strstr(file_content, "export LD_LIBRARY_PATH=\"$runtime_root/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}\"") != NULL);
    ...
    assert(strstr(file_content, "unzip -oq \"$EMU_DIR/pylibs.zip\" -d \"$EMU_DIR\"") != NULL);
    ...
    assert(strstr(file_content, "bind_flip_libmali() {\n    return 0\n}") != NULL);
    ...
    assert(strstr(file_content, "sed -i 's|/roms/ports/PortMaster|$XDG_DATA_HOME/PortMaster|g' \"$file\"") != NULL);
    assert(strstr(file_content, "sed -i 's|/mnt/sdcard/Roms/.portmaster/PortMaster|$XDG_DATA_HOME/PortMaster|g' \"$file\"") != NULL);
    ...
    assert(strstr(file_content, "local script_to_run=\"${PMI_PORT_SCRIPT:-$ROM_PATH}\"") != NULL);
    assert(strstr(file_content, "apply_port_arch_rewrites") != NULL);
    assert(strstr(file_content, "if launcher_requires_armhf \"$script_to_run\"; then") != NULL);
    assert(strstr(file_content, "seed_x86_runtime_libs \"$REAL_PORTS_DIR\"") != NULL);
    assert(strstr(file_content, "bind_flip_libmali") != NULL);
    assert(strstr(file_content, "stage_launch_script") != NULL);
    assert(strstr(file_content, "echo \"PMI_DIAG selected_port_script=$script_to_run\"") != NULL);
