...
int main(void) {
    ...
    assert(strstr(file_content, "if [ -f \"$cache_path\" ] &&") != NULL);
    assert(strstr(file_content, "! [ \"$inspect_path\" -nt \"$cache_path\" ] &&") != NULL);
    assert(strstr(file_content, "{ [ ! -e \"$checker\" ] || ! [ \"$checker\" -nt \"$cache_path\" ]; }; then") != NULL);
    assert(strstr(file_content, "if \"$checker\" needs-default-audio-info \"$inspect_path\"; then") != NULL);
    ...
    assert(strstr(file_content, "\"$checker\" uses-sdl-gl-windowing \"$file\"") != NULL);
    assert(strstr(file_content, "cache_path=$(printf '%s.pmi-system-gl-windowing\\n' \"$file\")") != NULL);
    assert(strstr(file_content, "! [ \"$file\" -nt \"$cache_path\" ] &&") != NULL);
    assert(strstr(file_content, "write_sdl_compat_check_cache \"$cache_path\" \"match\"") != NULL);
    assert(strstr(file_content, "echo \"PMI_WARN system_gl_checker_missing=$checker\"") != NULL);
    assert(strstr(file_content, "if command -v strings >/dev/null 2>&1 &&") != NULL);
    assert(strstr(file_content, "is_elf64_file \"$file\" &&") != NULL);
    assert(strstr(file_content, "strings \"$file\" 2>/dev/null | grep -qx 'SDL_GL_CreateContext' &&") != NULL);
