...
int main(void) {
    ...
    assert(strstr(file_content, "is_armhf_exec_wrapper()") != NULL);
    assert(strstr(file_content, "is_armhf_elf_binary()") != NULL);
    assert(strstr(file_content, "is_armhf_launch_binary()") != NULL);
    assert(strstr(file_content, "head -n 2 \"$file\" 2>/dev/null | grep -Eq '^# PMI_ARMHF_EXEC_COMPAT_WRAPPER=1$'") != NULL);
    assert(strstr(file_content, "set -- $(dd if=\"$file\" bs=1 count=20 2>/dev/null | od -An -tx1 -v 2>/dev/null)") != NULL);
    assert(strstr(file_content, "[ \"${19}\" = \"28\" ]") != NULL);
    assert(strstr(file_content, "case \"$base_name\" in") != NULL);
    assert(strstr(file_content, "*.sh|*.bash|*.so|*.so.*|*.original)") != NULL);
    ...
    assert(strstr(file_content, "uses_sdl_gl_windowing=\"${field3#uses_sdl_gl_windowing=}\"") != NULL);
    assert(strstr(file_content, "is_wrapper=\"${field4#is_wrapper=}\"") != NULL);
    ...
    assert(strstr(file_content, "is_armhf_launch_binary \"$file\" || continue") != NULL);
